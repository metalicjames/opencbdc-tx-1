...
namespace cbdc::threepc::agent::runner {
    auto tx_id(const std::shared_ptr<cbdc::threepc::agent::runner::evm_tx>& tx,
    ...
    auto
    tx_encode(const std::shared_ptr<cbdc::threepc::agent::runner::evm_tx>& tx,
              uint64_t chain_id,
        ...
        if(tx->m_to.has_value()) {
            to = make_rlp_value(tx->m_to.value(), true);
        }
