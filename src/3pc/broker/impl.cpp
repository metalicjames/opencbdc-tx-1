...
namespace cbdc::threepc::broker {
    ...
    auto impl::begin(begin_callback_type result_callback) -> bool {
        ...
        // TODO: add a TTL after which the TX must be committed or rolled back.
        ...
        auto result = std::visit(
            overloaded{
                [&](threepc::runtime_locking_shard::value_type v)
                    ...
                    if(k_it->second != key_state::locking) {
                        m_log->error("Shard state not locking");
                        return error_code::invalid_shard_state;
                    }

                    k_it->second = key_state::locked;

                    m_log->trace(this, "Broker locked key for", ticket_number);

                    return v;
                },
                [&](threepc::runtime_locking_shard::error_code e)
                    -> try_lock_return_type {
                    m_log->trace(this,
                                 "Shard error locking key for",
                                 ticket_number);
                    return e;
                }},
            res);
    ...
    auto impl::do_handle_prepare(
        ...
        if(res.has_value()) {
            if(res.value() != runtime_locking_shard::error_code::wounded) {
    ...
    auto impl::do_commit(const commit_callback_type& commit_cb,
        ...
        for(auto& shard : ts->m_shard_states) {
            ...
            if(!m_shards[sidx]->commit(
                   ticket_number,
                   [=](threepc::runtime_locking_shard::interface::
        ...
        auto maybe_error = [&]() -> std::optional<error_code> {
            ...
            for(auto& shard : t_state->m_shard_states) {
                for(auto& key : shard.second.m_key_states) {
                    if(key.second == key_state::locking) {
        ...
        for(auto& shard : t_state->m_shard_states) {
            ...
            if(!m_shards[shard.first]->prepare(
                   ...
                   [this,
                    ...
                    ticket_number,
                    shard_idx
                    = shard.first](threepc::runtime_locking_shard::interface::
    ...
    void impl::handle_find_key(
        ...
        auto maybe_error = [&]() -> std::optional<error_code> {
            ...
            tss->m_shard_states[res.value()].m_key_states[key]
                = key_state::locking;

            if(!m_shards[res.value()]->try_lock(
                   ticket_number,
                   m_broker_id,
                   key,
                   locktype,
